function kingPieceAction(state: any, pos: string) {
  const x = +pos.split('-')[0];
  const y = +pos.split('-')[1];
  ...
  const xStep = [0, 1, 2, 0, 1, 2, 0, 1, 2];
  const yStep = [0, 0, 0, 1, 1, 1, 2, 2, 2];
  ...
  const initialPos = () => {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      if (xMinusAbs < 2 && yMinusAbs < 2 && xMinusAbs !== yMinusAbs) {
        res.push({
          x: xOrigin,
          y: yOrigin,
        })          
...
export default kingPieceAction
